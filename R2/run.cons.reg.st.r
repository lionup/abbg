...
# 1, x, x^2-1, x^3-3x
# 0, 1*1, 2*x, 3*(x^2-1)
Mat3 <- array( 0, dim=c(nrow(nl_fu), (K1+1)*(K2+1)*(K3+1)) )
persis <- array( 0, dim=c(ntau, ntau) ) 
for (jtau1 in 1:ntau){
  for (jtau2 in 1:ntau){
    ...
    for (kk1 in 1:K1) {
      for (kk2 in 0:K2) {    
        for (kk3 in 0:K3) {
    		  Mat3[,1+kk3+(K3+1)*(kk2+(K2+1)*kk1)] = kk1 * hermite( nl_fu$ystd,kk1-1 ) / sdinc * 
    		    hermite(Vage[jtau1], kk2) * hermite(Vass[jtau2], kk3)
        }
      }  
    }
    persis[jtau1,jtau2] <- mean(Mat3 %*% ResP)
  }
}
