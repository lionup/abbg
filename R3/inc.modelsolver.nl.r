...
comp.eta.sim <- function(p){
  ...
  Mateta_true = array(0, dim = c(p$Twork,p$N))
  ...
  Mateta_true[1,] = Mateta_true[1,] + (MatAGE1 %*% p$Resqtrue_e0[,p$Ntau])*(V_draw[1,] > p$Vectau[p$Ntau])
...
comp.eta.prob <- function(p){
  ...
  xeta <- array( 0, dim=c(p$Twork, p$ngpz) )  #bins
  veta <- seq(1/(2*p$ngpz), (2*p$ngpz-1)/(2*p$ngpz), l=p$ngpz) #median of each bin
  age = seq(p$age_min, p$age_re-2, 2)

  for (i in 1:p$Twork){ #periods before retirement
    xeta[i,] <- quantile( Mateta_true[i,], veta, names=F, na.rm = T )
  }

  long <- data.table( pid = 1:p$N, age=rep(age, each=p$N), income = c(t(Mateta_true)) )
  setkey(long, pid, age)
  long[, q:=as.numeric(cut_number(income, n = p$ngpz)), age] #give a bin # for each person each age
  long$income <- NULL
  wide <- reshape(long, idvar='pid', timevar='age', direction='wide') #each person has all age in a row

  etaprob <- array( 0,dim=c(p$Twork, p$ngpz, p$ngpz) ) #last period to this period
  etaprob[1,,] <- 1/p$ngpz  #first period, same prob

  for (t in 2:p$Twork){ #today
  ...
  etacontot <- etaprob
  for ( i in 1:p$Twork ) {
    etacontot[i,,] <- econdCDF( p$ngpz, etaprob[i,,] )
  }
  ...
  mineta <- array( 0, dim=c(p$Twork, p$ngpz) ) #this period and last eta
  maxeta <- array( p$ngpz+1, dim=c(p$Twork, p$ngpz) )
  for(t in 2:p$Twork){ #today
    for(w in 1:p$ngpz){   #for yesterday
      for(ww in 1:p$ngpz){ #for today
        if (etaprob[t,w,ww] > 0) {
          break
        }else{
          mineta[t,w] = ww
...
comp.eps <- function(p, ntra){
  xeps <- array( 0, dim=c(p$Twork, ntra) ) #income grid
  VecTaue <- seq(1/(1+p$ngpe), p$ngpe/(1+p$ngpe), l=ntra)
  #VecTaue <- (1:ntra) / (1+ntra) #get the quantile of interpolation node

  # generate the age vector
  xt  <- seq( p$age_min, (p$age_min + (p$Twork-1) * 2 ), by=2 )
