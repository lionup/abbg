...
load( paste('~/git/abbg/R3/',names,'.dat',sep='') )
...
Mat <- mclapply(1:nrow(Kgrid), decomHerm, Kgrid, nl_fu, mc.cores = chainN )
...
persis2 <- mclapply(1:nrow(Vgrid), derivHerm, K1, K2, K3, nl_fu$ystd, sdinc, nobs, Vgrid, ResP, mc.cores = chainN )
