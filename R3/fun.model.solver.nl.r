source('inc.modelsolver.nl.r')

comp.ngpm <- function(im, p, model){
	source('inc.modelsolver_nl_kg.r')

	Mea <- array(0, dim=c(p$ngpz, p$ngpa+1))
	Cea <- Mea
	for ( e in 1:p$ngpz){
		Vap=0
		for ( i in 1:p$ngpe ){
			for ( j in (model$mineta[p$l+1,e] + 1) : (model$maxeta[p$l+1,e] - 1) ){
				...
				Vap  = Vap + 1/p$ngpe * model$etaprob[p$l+1,e,j] *
		...
		ChiVec = (p$R * p$bet * Vap)^(-1/p$gam) # inverse Euler equation
...
comp.solveModel <- function(p) {
	...
	#eps: grid and distri = 1/ngpe
	xeps = comp.eps(p, p$ngpe)
	...
	ygrid = array(0,dim=c(p$Twork,p$ngpz,p$ngpe))
	for(it in 1:p$Twork){
	  for(iz in 1:p$ngpz){
    	for(ie in 1:p$ngpe){
  ...
  zsimI <- array( 0,dim=c(p$nsim,p$Twork) )
  esimI <- zsimI
  ysim  <- zsimI
  yavsim<- zsimI
  #simulate to get distribution of average pre-tax incomes
  zsimI[,1] = sample(1:p$ngpz,p$nsim,replace=T)
  esimI[,1] = sample(1:p$ngpe,p$nsim,replace=T)

	for (i in 1:p$nsim){
		ysim[i,1] = ygrid[ 1,zsimI[i,1],esimI[i,1] ]
		yavsim[i,1]= ysim[i,1]
	  for (it in 2:p$Twork){
	     zsimI[i,it] = sample( 1:p$ngpz,1,prob=etaprob[it,zsimI[i,it-1],] )
	     esimI[i,it] = sample( 1:p$ngpe,1)
  ...
  # create last period C and M after retirement
	...
	# create C and M before retirement
 	C = array( 0, dim=c(p$Twork,p$ngpm,p$ngpz,p$ngpa+1) )
	M = C
	...
	for ( im in 1:p$ngpm){
		...
		ChiVec = (p$R * p$bet * Vap)^(-1/p$gam) # inverse Euler equation
		MuVec  = agrid + ChiVec
		M[p$Twork,im,,]  = array( rep(c(0, MuVec),each=p$ngpz),dim=c(p$ngpz,p$ngpa+1) )    # Matrix of interpolation data
		C[p$Twork,im,,]  = array( rep(c(0,ChiVec),each=p$ngpz),dim=c(p$ngpz,p$ngpa+1) )    # Matrix of interpolation data
	}
...
comp.moments<- function(p, model) {
  ...
  set.seed(77)
	...
	# declare variable
	etasimI <- array( 0,dim=c(p$nsim,p$Twork) )
  epssimI <- etasimI
  etasim  <- etasimI
  epssim  <- etasimI
  yavsim	<- etasimI
  msimI   <- etasimI
  msim    <- etasimI
  ysim    <- array( 0,dim=c(p$nsim,p$Twork+p$Tret) )
  asim    <- ysim
  xsim    <- ysim
  csim    <- ysim
  #simulate to get distribution of average pre-tax incomes
  etasimI[,1] = sample(1:p$ngpz,p$nsim,replace=T)
  epssimI[,1] = sample(1:p$ngpe,p$nsim,replace=T)
  ...
  for (i in 1:p$nsim){
	...
	for (it in 2:p$Twork) {
    epssimI[,it] = sample(1:p$ngpe,p$nsim,replace=T)
    epssim[,it]  = xeps[ cbind(it,epssimI[,it]) ]

    etasimI[,it] = sapply(1:p$nsim, function(x)
    	sample(1:p$ngpz, 1, prob=etaprob[it, etasimI[x,it-1],]) )
	...
	##RETIRE####
  for (it in 1:p$Tret) {
  	ysim[,p$Twork+it]  = pgrid[ msimI[,p$Twork] ]
  	asim[,p$Twork+it]  = p$R * ( xsim[,p$Twork+it-1] - csim[,p$Twork+it-1] )
  	xsim[,p$Twork+it]  = asim[,p$Twork+it] + ysim[,p$Twork+it]

    for (i in 1:p$nsim){
      csim[i,p$Twork+it] = approx( Mret[it, msimI[i,p$Twork],],
                                  Cret[it, msimI[i,p$Twork],],
                                  xsim[i,p$Twork+it] )$y
    }
  }
